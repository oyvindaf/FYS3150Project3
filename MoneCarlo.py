...
def MonteCarloNormal(N,M):

    points=np.random.uniform(0,1,(N,M))
    func_values=np.exp(points)
    mean_value=np.mean(func_values,axis=0)
    standard_dev=np.sqrt(np.sum(func_values-mean_value,axis=0)**2)/(N*M)
    total_mean_value=np.sum(mean_value)
    total_standard_dev=np.sum(standard_dev)
    return total_mean_value,total_standard_dev

def MonteCarloExponential(N,M):

    points=np.random.exponential((N,M))
    func_values=np.exp(points)
    mean_value=np.mean(func_values)
    standard_dev=np.sqrt(np.sum(func_values-mean_value)**2)/(N*M)
    return mean_value,standard_dev


mean_value,standard_deviation=MonteCarloNormal(1000000,10)
print(mean_value)
